...
public class Robot extends TimedRobot {
  ...
  */
  ...
  public void autonomousPeriodic() {
    ...
    if (m_timer.get() > 0.0 && m_timer.get() < 1.0) { //DRIVE FORWARD, 2 SECONDS
      leftLeader.set(autoFwd);
      rightLeader.set(autoFwd);
    }
    else if (m_timer.get() > 1.0 && m_timer.get() < 5.0) { //BETWEEN 2 AND 10 SECONDS, MOVE TO FACE 90 DEGREES
        if (heading < 119.0) {
          leftLeader.set(autoRot);
          rightLeader.set(-autoRot);
        }
        else if (heading > 121.0) {
          leftLeader.set(-autoRot);
          rightLeader.set(autoRot);
        }
    }
